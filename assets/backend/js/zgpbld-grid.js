(function($){
		...
		g.ox = e.pageX;	 //drag position
                ...
                $(e.target).closest('.zgpb-fl-gridsystem-opt')
                        .find('.zgpb-fl-gd-opt-icon-handler')
                        .css('background','#CEFBF8');
                
                //attach events
                
                $(document).bind('mousemove.'+ID, obj.drag_move).bind('mouseup.'+ID,obj.drag_over);	//mousemove and mouseup events are bound
                $("head").append("<style id='zgpb-grid-head-css' type='text/css'>*{cursor:e-resize!important;   -webkit-touch-callout: none;-webkit-user-select: none;-khtml-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;}</style>"); 	//change the mouse cursor
                drag = g;
            ...
            this.drag_move = function(e){
             ...
             var syncCols = function(t,i,right){
                 ...
                 if(right===true){
                      ...
                      w = c.w + inc;	var w2= c2.w- inc;	//their new width is obtained
                  ...
                  if(right===true ){
                            if(parseFloat(w)>temp_col2){
                                ...
                                if(($.inArray(c_new_igrid,possible_index)>-1)
                                    && ($.inArray(c2_new_igrid,possible_index)>-1)){
