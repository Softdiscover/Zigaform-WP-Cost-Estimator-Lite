...
class Uiform_InstallDB {
    ...
    public function install($networkwide = false){
       ...
       if( isset($uifm_check_total['total']) && intval($uifm_check_total['total'])===0 ){
           $sql="INSERT INTO $this->settings VALUES ('3.9.9.8', null, null, null, null, null, null, '', '1');";
        $wpdb->query($sql);
       }
   ...
   //form log
        ...
        $sql="CREATE  TABLE IF NOT EXISTS $this->core_addon (
         ...
         if(ZIGAFORM_C_LITE!=1){
           ...
           if( isset($uifm_check_total['total']) && intval($uifm_check_total['total'])===0 ){
              $sql="INSERT INTO $this->core_addon VALUES ('func_anim', 'Animation effect', 'Animation effects to fields', 1, 1, NULL, NULL, NULL, 1, NULL, NULL, NULL, 1, '1980-01-01 00:00:01', '2018-01-31 10:35:14', NULL, NULL, NULL, NULL, NULL, 1, 1, 1);";
                $wpdb->query($sql);
            }
         }
