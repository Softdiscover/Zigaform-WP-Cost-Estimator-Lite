...
class CaptchaBuilder implements CaptchaBuilderInterface
{
    ...
    protected function writePhrase($image, $phrase, $fonts, $width, $height)
    {
        ...
        $captcha_size = array ($width,$height);
        ...
        for ($i = 0; $i < $length; $i++) {
            //random color
            $fcolor = imagecolorallocate($image, rand(150, 250), rand(0, 120), rand(0, 120));
            //random size
            $tsize = $fonts[rand(0, count($fonts) - 1)];
            //$tsize = $font_src;
            //random position
            $ypos = mt_rand(0, $height - imagefontheight($font_src));
            $xpos = $i * imagefontwidth($font_src)+10;
            //random code
            
            imagechar($image, $tsize, $xpos, $ypos,  $phrase[$i], $fcolor);
     ...
     * Generate the image
     */
    public function build($width = 100, $height = 40, $font = null, $fingerprint = null)
    {
        if (null !== $fingerprint) {
        ...
        if ($font === null) {
            ...
            $font1 = imageloadfont(__DIR__ . '/Font/font.gdf');
            $font2 = imageloadfont( __DIR__ . '/Font/font2.gdf');
            $font3 = imageloadfont( __DIR__ . '/Font/font3.gdf');
            $fonts = array($font1, $font2, $font3 );
        }
    ...
    public function output($quality = 90)
    {
        ...
        imagepng($this->contents);
        imagedestroy($this->contents);
